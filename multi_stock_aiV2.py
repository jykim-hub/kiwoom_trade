...
def analyze_stock(ticker):
    ...
    # XGBoost
    features = ['Bullish_Sum_5', 'Bearish_Sum_5', 'RSI', '%K', 'Volume', 'ATR', 'OBV', 'OBV_Change', 'Return_3']
    data = data.dropna()
    X = data[features]
    y = data['Price_Label']
    ...
    # 백테스트 (스탑로스 및 트레일링 스탑, 심리적 거래량 조건)
    initial_cash = 100000
    cash = initial_cash
    shares = 0
    trades = []
    ...
    for date, row in data.iterrows():
        if row['BUY_Prob'] > 0.75 and row['Volume'] > row['Volume_MA'] * 2 and cash > row['Close']:
            ...
            if (row['SELL_Prob'] > 0.75 and row['Volume'] > row['Volume_MA'] * 2) or \
               row['Close'] < buy_price * 0.95 or row['Close'] < max_price * 0.97:
    ...
    return {
        ...
        'accuracy': accuracy,
        'return': return_,
        'recent_probs': data[['Close', 'RSI', 'Volume', 'SELL_Prob', 'BUY_Prob']].tail(5),
