...
def load_or_download_data(ticker, start_date, end_date, csv_file):
    ...
    if os.path.exists(csv_file):
        try:
            ...
            if pd.notna(last_date) and last_date >= end_date - pd.Timedelta(days=1):
                print(f"{csv_file} is up-to-date.")
                return data
            
            print(f"Updating {csv_file} from {last_date + pd.Timedelta(days=1)} to {end_date}...")
            new_data = yf.download(ticker, start=last_date + pd.Timedelta(days=1), end=end_date, auto_adjust=False)
            if not new_data.empty:
                if isinstance(new_data.columns, pd.MultiIndex):
                    new_data.columns = [col[0] for col in new_data.columns]
                new_data.columns = [col.title() for col in new_data.columns]
                data = pd.concat([data, new_data])
                data = data[~data.index.duplicated(keep='last')]
                data.to_csv(csv_file)
                return data
            else:
                print(f"No new data available for {ticker}. Using cached data.")
                return data
                
        except Exception as e:
            print(f"Error loading {csv_file}: {e}. Downloading fresh data...")
...
def analyze_stock(ticker):
    ...
    def calculate_rsi(data, periods=14):
        ...
        rs = gain / loss
        rs = rs.replace([np.inf, -np.inf], 0)  # inf 처리
        rsi = 100 - (100 / (1 + rs))
        return rsi
    ...
    data['Volume_Signal'] = np.where(data['Volume'] > data['Volume_MA'] * 2, 1, 0)

    # Stochastic Oscillator
    data['L14'] = data['Low'].rolling(window=14).min()
    data['H14'] = data['High'].rolling(window=14).max()
    data['%K'] = 100 * (data['Close'] - data['L14']) / (data['H14'] - data['L14'])
    data['%K'] = data['%K'].replace([np.inf, -np.inf], 0)  # inf 처리
    ...
    data['OBV_Change'] = data['OBV'].pct_change(periods=5)
    data['OBV_Change'] = data['OBV_Change'].replace([np.inf, -np.inf], 0)  # inf 처리
    ...
    data['Bullish_Sum_10'] = data['Bullish_Count'].rolling(window=10).sum()
    data['Bearish_Sum_10'] = data['Bearish_Count'].rolling(window=10).sum()
    
    # 변화율 계산 시 inf 처리
    data['Bullish_Sum_10_Change'] = data['Bullish_Sum_10'].pct_change(periods=10)
    data['Bullish_Sum_10_Change'] = data['Bullish_Sum_10_Change'].replace([np.inf, -np.inf], 0)
    data['Bearish_Sum_10_Change'] = data['Bearish_Sum_10'].pct_change(periods=10)
    data['Bearish_Sum_10_Change'] = data['Bearish_Sum_10_Change'].replace([np.inf, -np.inf], 0)
    ...
    # LSTM 입력 시퀀스 생성
    features = ['Bullish_Sum_10_Change', 'Bearish_Sum_10_Change', 'RSI', '%K', 'Volume', 'ATR', 'OBV', 'OBV_Change', 'Return_3']
    data = data.dropna()
    
    # 입력 데이터에서 inf/NaN 확인 및 제거
    X = data[features].values
    mask = np.all(np.isfinite(X), axis=1)
    X = X[mask]
    y = data['Price_Label'].values[mask]
    
    if len(X) < time_steps:
        raise ValueError(f"Insufficient data after cleaning: {len(X)} rows, need at least {time_steps}")
    ...
    for date, row in data.iterrows():
        if row['BUY_Prob'] > 0.75 and row['Volume'] > row['Volume_MA'] * 2 and cash > row['Close']:
            shares_to_buy = int(cash * 0.1 // row['Close'])  # 최대 10% 투자
            if shares_to_buy > 0:
                cash -= shares_to_buy * row['Close'] * (1 + commission_rate)
            ...
            if (row['SELL_Prob'] > 0.75 and row['Volume'] > row['Volume_MA'] * 2) or \
               row['Close'] < buy_price * 0.95 or row['Close'] < max_price * 0.97 or \
               row['RSI'] > 70:  # RSI 기반 매도 조건 추가
                cash += shares * row['Close'] * (1 - commission_rate)
                trades.append({'date': date, 'type': 'SELL', 'price': row['Close'], 'shares': shares})
                shares = 0
                buy_price = 0
                max_price = 0
    final_value = cash + shares * data['Close'].iloc[-1] * (1 - commission_rate)
    return_ = (final_value - initial_cash) / initial_cash * 100
    csv_outfile = os.path.join(data_dir, f"{ticker}_outdata.csv")
    # 결과 저장
    data.to_csv(csv_outfile)
    ...
    chart_data = {
        ...
        "data": {
            ...
            "datasets": [
                ...
                {"label": "RSI", "data": data['RSI'][-30:].tolist(), "borderColor": "#8b5cf6", "fill": False, "yAxisID": "y1"},
                {"label": "Volume", "data": data['Volume'][-30:].tolist(), "borderColor": "#f59e0b", "fill": False, "yAxisID": "y2"}
            ]
        },
        "options": {
            "scales": {
                ...
                "y2": {"beginAtZero": False, "position": "right", "grid": {"drawOnChartArea": False}}
            }
        }
    }
    ...
    return {
        ...
        'accuracy': accuracy,
        'return': return_,
        'recent_probs': data[['Close', 'RSI', 'Volume', 'Bullish_Sum_10_Change', 'Bearish_Sum_10_Change', 'SELL_Prob', 'BUY_Prob']].tail(5),
...
# 5. AAPL 최근 5일 결과
print("\nAAPL 최근 5일 BUY/SELL 확률 및 정서 지표:")
